...
// Robot parameters and variables
...
int main(void)
{
  ...
  // Timer and interrupt pin INT0 init
  timer2_init();
  int0_init();
  sei();

  // init and set I2C and OLED
  twi_init();
  ...
  while (1)
  {
    ...
    if (robot_run == 1)
    {
      S1 = map(analog_read(SENSOR_CR), S1K[1], S1K[0], 0, 100);   // Read and normalize analog values
    ...
    else    // stopping the robot (robot_run = 0)
    {
      pwm_write(&PORTD, MOTOR_LF, 0);
      pwm_write(&PORTD, MOTOR_RF, 0);
    ...
    if (gpio_read(&PIND, BUTTON) == 0)    // is button pressed?
    {
      debounce();
      robot_run = !robot_run;  // toggle robot state (run/stop)
    }
  }
  return 0;
}
...
void auto_calibration()
{
  ...
  S1K[0] = S1K[1] = analog_read(SENSOR_CR);   // Read analog values from IR sensor
  ...
  while (calib_tick < 150) // calibration loop
  {
    ...
    // Search for max (white) and min (black) value
    // SxK[0] - white
    // SxK[1] - black
...
ISR(INT0_vect)
{
  ...
  if (bit_index == frame_length)    // received all frames?
  {
...
static int16_t constrain(int16_t val, uint8_t min, uint8_t max)   // cropp value
{
  uint8_t motor = 0;
  if (val <= min)
    motor = min;
...
static inline int16_t map(int16_t x, int16_t in_min, int16_t in_max, int16_t out_min, int16_t out_max)    // Normalize values
{
